...
class DeductionService {
  ...
  // Generic fetch method with cache control and better error handling
  private async fetchWithCache<T>(
    ...
    try {
      ...
      // Only add timestamp if not in params already
      if (!params?._t) {
        urlParams.append('_t', Date.now().toString());
      }

      const url = `${API_BASE_URL}/${endpoint}${urlParams.toString() ? `?${urlParams}` : ''}`;
      ...
      // Return null or throw based on the type of endpoint
      // For critical data, throw error; for stats/optional data, return null
      if (endpoint.includes('stats') || endpoint.includes('export')) {
        return null;
      }
      
      // Throw error for main data endpoints
      throw error;
    } finally {
  ...
  async getEmployees(params?: {
    ...
    try {
      ...
      if (!data) {
        // Return empty successful response if fetch fails
        return {
          success: true,
          data: []
        };
  ...
  async getDeductionById(id: string): Promise<ApiResponse<Deduction>> {
    try {
      const response = await fetch(`${API_BASE_URL}/deductions/${id}?_t=${Date.now()}`, {
        headers: {
          'Cache-Control': 'no-cache, no-store, must-revalidate',
          'Pragma': 'no-cache',
          'Accept': 'application/json'
  ...
  async createBulkDeductions(deductions: CreateDeductionRequest[]): Promise<ApiResponse<Deduction[]>> {
    try {
      const response = await fetch(`${API_BASE_URL}/deductions/bulk`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Cache-Control': 'no-cache',
  ...
  async updateDeductionStatus(id: string, status: Deduction['status']): Promise<ApiResponse<Deduction>> {
    try {
      const response = await fetch(`${API_BASE_URL}/deductions/${id}/status`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Cache-Control': 'no-cache',
  ...
  async exportDeductions(params?: {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/deductions/export?${urlParams}`, {
        headers: {
          'Cache-Control': 'no-cache, no-store, must-revalidate',
          'Pragma': 'no-cache'
        }
